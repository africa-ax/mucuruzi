...
const AuthView = (() => {
    ...
    _root().innerHTML = _shell(`
      ...
      <div class="form-group">
        ...
        <div class="input-password-wrap">
          <input id="login-password" type="password" class="form-input" placeholder="Your password" autocomplete="current-password" />
      ...
      <button id="login-btn" class="btn btn-primary btn-block btn-lg" onclick="AuthView._handleLogin()">
        Sign In
      </button>
      ...
      <p class="auth-switch">
        ...
        <a href="#" onclick="AuthView.showRegister(); return false;">Register here</a>
      </p>
    `);
  ...
  const showRegister = () => {
    _root().innerHTML = _shell(`
      ...
      <div class="form-group">
        ...
        <div class="role-selector" id="role-selector">
          ${Object.entries(ROLE_LABELS).map(([val, label]) => `
            <button type="button"
              class="role-btn ${val === 'buyer' ? 'active' : ''}"
              data-role="${val}"
              onclick="AuthView._selectRole('${val}')">
      ...
      <div class="form-group">
        <label class="form-label">Email</label>
        <input id="reg-email" type="email" class="form-input" placeholder="your@email.com" autocomplete="email" />
      </div>
      ...
      <!-- Seller-Only Fields (hidden for buyer) -->
      ...
      <div class="form-group auth-terms">
        <label class="checkbox-label">
          ...
          <span>
            I agree to the
            <a href="#terms" onclick="AuthView._openTerms(); return false;">Terms & Conditions</a>
            and
            <a href="#privacy" onclick="AuthView._openPrivacy(); return false;">Privacy Policy</a>
          </span>
        </label>
      </div>

      <button id="register-btn" class="btn btn-primary btn-block btn-lg" onclick="AuthView._handleRegister()">
        Create Account
      </button>

      <p class="auth-switch">
        Already have an account?
        <a href="#" onclick="AuthView.showLogin(); return false;">Sign in</a>
      </p>
    `);
  };
  ...
  const showForgotPassword = () => {
    _root().innerHTML = _shell(`
      ...
      <div class="form-group">
        <label class="form-label">Email</label>
        <input id="reset-email" type="email" class="form-input" placeholder="your@email.com" autocomplete="email" />
      </div>

      <button id="reset-btn" class="btn btn-primary btn-block btn-lg" onclick="AuthView._handleReset()">
        Send Reset Link
      </button>

      <p class="auth-switch">
        <a href="#" onclick="AuthView.showLogin(); return false;">← Back to Sign In</a>
      </p>
    `);
  ...
  const _handleLogin = async () => {
    ...
    if (!email || !password) {
      return _showError('Please enter your email and password.');
    }
    ...
    // On success: app.js onAuthStateChanged handles redirect
  };

  const _handleGoogle = async () => {
    _setLoading(null, true);
    const result = await AuthService.loginWithGoogle();
    _setLoading(null, false);

    if (!result.success) {
      if (result.needsRegistration) {
        _showError(result.error + ' Click "Register here" to create an account.');
      } else {
        _showError(result.error);
      }
    }
  ...
  const _handleRegister = async () => {
    ...
    if (isSeller) {
      if (!tinNumber || !/^\d{9}$/.test(tinNumber)) {
        return _showError('TIN must be exactly 9 digits.');
      }
    ...
    const result = await AuthService.register({
      ...
      phone, district, tinNumber, sdcId,
    });
  ...
  const _selectRole = (role) => {
    ...
    if (sellerFields) {
      sellerFields.classList.toggle('hidden', !ROLE_CAN_SELL.includes(role));
    }
  };
  ...
  // Terms & Privacy — pages built later
  const _openTerms   = () => Toast.show('Terms & Conditions page coming soon.', 'info');
  const _openPrivacy = () => Toast.show('Privacy Policy page coming soon.', 'info');
