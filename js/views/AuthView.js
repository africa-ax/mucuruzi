...
const AuthView = (() => {
  ...
  const _root = () => {
    // Auth screens render full screen — use app-root directly
    // Clear app-content so auth page is not inside the layout wrapper
  ...
  const showRegister = () => {
    _root().innerHTML = _shell(`
      ...
      <!-- Manufacturer sub-type: shown only when manufacturer is selected -->
      <div id="manufacturer-subtype" class="hidden">
        <div class="form-group">
          <label class="form-label">Business Type</label>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-sm)">
            <button type="button" class="role-btn active" id="subtype-producer"
              onclick="AuthView._selectSubtype('producer')">
              <span class="role-btn-icon">🏭</span>
              <span>Producer</span>
            </button>
            <button type="button" class="role-btn" id="subtype-importer"
              onclick="AuthView._selectSubtype('importer')">
              <span class="role-btn-icon">🚢</span>
              <span>Importer</span>
            </button>
          </div>
          <input type="hidden" id="reg-business-type" value="producer" />
        </div>
      </div>
  ...
  const _handleLogin = async () => {
    ...
    // Login succeeded — set up layout and navigate immediately
    window.currentUser = result.user;
    Navbar.render(result.user);
    Sidebar.render(result.user);
  ...
  const _handleGoogle = async () => {
    ...
    // Google login succeeded — set up layout and navigate immediately
    window.currentUser = result.user;
    Navbar.render(result.user);
    Sidebar.render(result.user);
    Router.navigate('#dashboard');
  ...
  const _handleRegister = async () => {
    ...
    const businessType = document.getElementById('reg-business-type')?.value || 'producer';
    ...
    if (ROLE_NEEDS_TIN.includes(role)) {
    ...
    try {
      result = await AuthService.register({
        email, password, businessName, role, phone, district, tinNumber, sdcId,
        businessType: role === ROLES.MANUFACTURER ? businessType : null,
      });
    } catch(e) {
      result = { success: false, error: e.message || 'Unexpected error. Please try again.' };
    }
    ...
    // Because _isRegistering flag blocked onAuthStateChanged,
    // we must manually set up layout and navigate to dashboard.
    window.currentUser = result.user;
    Navbar.render(result.user);
    Sidebar.render(result.user);
    Toast.success(`Welcome to Mucuruzi, ${result.user.businessName}!`);
    Router.navigate('#dashboard');
  };
  ...
  const _selectRole = (role) => {
    ...
    // Show TIN/SDC for roles that need it
    const sf = document.getElementById('seller-fields');
    if (sf) sf.classList.toggle('hidden', !ROLE_NEEDS_TIN.includes(role));
    // Show manufacturer sub-type only for manufacturer
    const mst = document.getElementById('manufacturer-subtype');
    if (mst) mst.classList.toggle('hidden', role !== ROLES.MANUFACTURER);
  };

  const _selectSubtype = (subtype) => {
    const input = document.getElementById('reg-business-type');
    if (input) input.value = subtype;
    const producer = document.getElementById('subtype-producer');
    const importer = document.getElementById('subtype-importer');
    if (producer) producer.classList.toggle('active', subtype === 'producer');
    if (importer) importer.classList.toggle('active', subtype === 'importer');
  };
  ...
  return {
    ...
    _selectRole, _selectSubtype, _togglePassword, _openTerms, _openPrivacy,
  };
